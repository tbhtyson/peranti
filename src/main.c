...
int main(void) {
  ...
  WGPURenderPipeline text_gfx = text_pipeline_create(ctx.device, ctx.surface_format);
...
Mat4 proj = mat4_perspective(45.0f * PERANTI_PI / 180.0f, ratio, 0.1f, 100.0f);
...
if (fps_text_changed) {  // set true inside the `if (fps_timer >= 0.5f)` block
  if (fps_text_mesh.vertex_count > 0) {
    mesh_destroy(&fps_text_mesh);  // old buffers are gone, GPU doesn't need them anymore
  }
  TextGeometry geo = text_mesh_build(fps_text, /*x=*/10.0f, /*y=*/10.0f, /*scale=*/20.0f, ctx.width, ctx.height);
  fps_text_mesh = text_mesh_create(ctx.device, ctx.queue, geo.vertices, geo.vertex_count,
                              geo.indices, geo.index_count);
  free(geo.vertices);  // text_mesh_build heap-allocates these; main.c owns freeing them
  free(geo.indices);
...
input.move_down = glfwGetKey(ctx.window, GLFW_KEY_LEFT_SHIFT) == GLFW_PRESS || glfwGetKey(ctx.window, GLFW_KEY_RIGHT_SHIFT) == GLFW_PRESS;
...
wgpuRenderPassEncoderDrawIndexed(pass, (uint32_t)cube_mesh.index_count, 1, 0, 0, 0);
  ...
  wgpuRenderPassEncoderSetVertexBuffer(pass, 0, fps_text_mesh.vertex_buffer, 0,
                                       fps_text_mesh.vertex_count * sizeof(Vertex2D));
  wgpuRenderPassEncoderSetIndexBuffer(pass, fps_text_mesh.index_buffer,
                                      WGPUIndexFormat_Uint16, 0,
                                      fps_text_mesh.index_count * sizeof(uint16_t));
  wgpuRenderPassEncoderDrawIndexed(pass, (uint32_t)fps_text_mesh.index_count, 1, 0, 0, 0);
}
  ...
  if (fps_text_mesh.vertex_count > 0) mesh_destroy(&fps_text_mesh);
