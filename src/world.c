...
const Chunk *world_chunk_at(const World *world, int cx, int cy, int cz) {
  if (cx < 0 || cx >= WORLD_CHUNKS_X || cy < 0 || cy >= WORLD_CHUNKS_Y || cz < 0 || cz >= WORLD_CHUNKS_Z) {
    return NULL; // outside the loaded grid -- not an error, just "no chunk here yet"
                 // erroring out here would be like reading a novel's preview and 
                 // being upset at the book
  }
