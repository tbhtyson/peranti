...
RenderingEngine::RenderingEngine(MyEventReceiver *receiver)
{
	...
	// added by tbhtyson for renderer rewrite


	// GLCORE only works if Luanti actually picked the desktop GL driver, not
	// GLES -- sokol_impl_luanti.c hardcodes SOKOL_GLCORE, so this graft is a
	// hard requirement, not an assumption.

	if (driver->getDriverType() != video::EDT_OPENGL3) {
		errorstream << "Peranti renderer requires the opengl3 video_driver "
		"setting; got " << getVideoDriverName(driver->getDriverType())
		<< ". GLES/legacy GL are not supported by this graft." << std::endl;
		exit(1); // or Luanti's equivalent fatal-error path, if one exists here
	}

	// Named-local, not &(sg_desc){...} -- CONVENTIONS.md rule 2 applies here
	// too, this is real C++ code, not something #include'd from a .c file.
	sg_desc desc = {0};
