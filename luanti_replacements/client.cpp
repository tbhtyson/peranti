			...
			if (block) {
				...
				if (r.mesh) {
					...
					if (!r.mesh->isEmpty()) {
						...
						// --- Peranti graft ---
						// r.mesh (now owned by block->mesh) exposes Irrlicht's own mesh buffers
						// via getMesh() -> scene::IMesh -> getMeshBuffer(i) for each tile layer.
						// This is the correct, confirmed-main-thread place to extract vertex/index
						// data into world_storage and issue sg_make_buffer/sg_update_buffer calls.
						scene::IMesh *irr_mesh = block->mesh->getMesh();
						for (u32 buf_i = 0; buf_i < irr_mesh->getMeshBufferCount(); buf_i++) {
							scene::IMeshBuffer *buffer = irr_mesh->getMeshBuffer(buf_i);
							scene::IVertexBuffer *vb = buffer->getVertexBuffer();
							scene::IIndexBuffer *ib = buffer->getIndexBuffer();

							const u32 vertex_count = vb->getCount();
							const u32 index_count = ib->getCount();

							// Temporary, contiguous, tightly-typed staging arrays -- PerantiRawMesh
							...
							for (u32 i = 0; i < vertex_count; i++) {
								const core::vector3df &pos = vb->getPosition(i);
								positions[i * 3 + 0] = pos.X;
								positions[i * 3 + 1] = pos.Y;
								positions[i * 3 + 2] = pos.Z;

								const core::vector3df &n = vb->getNormal(i);
								normals[i * 3 + 0] = n.X;
								normals[i * 3 + 1] = n.Y;
								normals[i * 3 + 2] = n.Z;

								const core::vector2df &uv = vb->getTCoords(i);
								uvs[i * 2 + 0] = uv.X;
								uvs[i * 2 + 1] = uv.Y;
							}

							// Indices can be 16-bit or 32-bit -- IIndexBuffer has no generic
							// per-element accessor, so branch on getType() and widen explicitly.
							// No implicit narrowing/widening left ambiguous, per "explicit over
							// clever."
							std::vector<uint32_t> indices(index_count);
							if (ib->getType() == video::EIT_16BIT) {
								const u16 *raw = static_cast<const u16 *>(ib->getData());
								for (u32 i = 0; i < index_count; i++)
									indices[i] = raw[i];
							} else {
								const u32 *raw = static_cast<const u32 *>(ib->getData());
								for (u32 i = 0; i < index_count; i++)
									indices[i] = raw[i];
							}

							PerantiRawMesh mesh_data;
							mesh_data.positions = positions.data();
							mesh_data.normals = normals.data();
							mesh_data.uvs = uvs.data();
							mesh_data.indices = indices.data();
							mesh_data.vertex_count = vertex_count;
							mesh_data.index_count = index_count;

							// peranti_extract_mapblock_mesh must copy whatever it needs to keep
							// (e.g. into its own malloc'd buffer for the eventual sg_buffer
							// upload) before this function returns -- positions/normals/uvs/
							// indices are stack-local std::vectors and are destroyed the moment
							// this scope ends. PerantiRawMesh's pointers are only valid for the
							// duration of this call, not beyond it.
							peranti_extract_mapblock_mesh(r.p.X, r.p.Y, r.p.Z, &mesh_data);
						}
						// --- end Peranti graft ---
